...
const Explore = () => {
  ...
  const {data:searchedPosts,isFetching: isSearchFetching}=useSearchPosts(debouncedVal)  
    ...
    <div className='explore-container'>
      ...
      <h3 className='body-bold md:h3-bold'>Popular Today</h3>
       ...
       <div className='flex-center gap-3 bg-dark-3 rounded-xl px-4 py-2 cursor-pointer'>
          <p className='small-medium md:base-medium text-light-2'>All
       ...
       <div className="flex flex-wrap md:gap-6 lg:gap-6 w-full max-w-5xl">
         {shouldShowSearchResults?
          ...
          :shouldShowPosts? (<p className='text-light-4 mt-10 text-center w-full'>End of posts</p>):
            (posts.pages.map((item, index)=>{
             return <GridPostList key={`page-${index}`} posts={item?.documents }  showUser={true} showStats={true}/>
            }))
        } 
       </div>
